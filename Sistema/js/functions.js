 $(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function(response){
                if(response != 'error'){
                    ...
                    $('.bodyModal').html('<form action="addProduct" method="post" name="form_add_product" id="form_add_product" onsubmit="event.preventDefault(); sendDataProduct();">'+
                                            '<h1>Agregar Producto</h1>'+
                                            '<h2 class="nameProduct">'+info.descripcion+'</h2><br>'+
                                            '<input type="number" name="cantidad" id="txtCantidad" placeholder="Cantidad a agregar" required>'+
                                            '<input type="number" name="precio" id="txtPrecio" min="0" step="0.10" style="margin: 5px 0;" value="'+info.precio_compra+'" placeholder="Precio de compra" required>'+
                                            '<input type="hidden" name="producto_id" id="producto_id"  value="'+info.codproducto+'"required>'+
                                            '<input type="hidden" name="action" value="addProduct">'+
                                            '<div class="alert alertAddProduct"></div>'+
                                            '<button type="submit" class="btn-new"><h3>Agregar <i class="fa fa-plus"></i></h3></button>'+
                                            '<a href="#" class="btn-ok closemodal" onclick="closemodal();"><h3>Cerrar <i class="fa fa-ban"></i></h3></a>'+ 
                                        '</form>');
    ...
    $(".del_product").click(function(e){
        ...
        $.ajax({
            ...
            success: function(response){
                if(response != 'error'){
                    var info = JSON.parse(response);
                    $('.bodyModal').html('<form action="delProduct" method="post" name="form_del_product" id="form_del_product" onsubmit="event.preventDefault(); delProduct();">'+
                                            '<h1>Eliminar Producto</h1>'+
                                            '<h3>¿Está seguro que desea eliminar el siguiente producto?</h3>'+
                                            '<h2 class="nameProduct">'+info.descripcion+'</h2><br>'+
                                            '<input type="hidden" name="del_producto_id" id="del_producto_id" value="'+info.codproducto+'"required>'+
                                            '<input type="hidden" name="action" value="delProduct" required>'+
                                            '<div class="alert alertDelProduct"></div>'+
                                            '<a href="#" class="btn-cancel" onclick="closemodal();"><i class="fas fa-ban"></i> Cancelar</a>'+
                                            '<button type="submit" class="btn-ok"><i class="far fa-trash-alt"></i> Eliminar</button>'+ 
                                        '</form>');
                }
            },
            error: function(error){
    ...
    $(".del_cliente").click(function(e){
        ...
        $.ajax({
            ...
            if(response != 'error'){
                    var info = JSON.parse(response);
                    if(info.dni!=''){
                    $('.bodyModal').html('<form action="delCliente" method="post" name="form_del_cliente" id="form_del_cliente" onsubmit="event.preventDefault(); delCliente();">'+
                                            '<h1>Eliminar Cliente</h1>'+
                                            '<h3>¿Está seguro que desea eliminar el siguiente cliente?</h3>'+
                                            '<h2 class="dniCliente">DNI: '+info.dni+'</h2><br>'+
                                            '<h2 class="nameCliente">Nombre: '+info.nombres+'</h2><br>'+
                                            '<input type="hidden" name="del_cliente_id" id="del_cliente_id" value="'+info.idcliente+'"required>'+
                                            '<div class="alert alertDelProduct"></div>'+
                                            '<a href="#" class="btn-cancel" onclick="closemodal();"><i class="fas fa-ban"></i> Cancelar</a>'+
                                            '<button type="submit" class="btn-ok"><i class="far fa-trash-alt"></i> Eliminar</button>'+ 
                                        '</form>');
                    ...
                    $('.bodyModal').html('<form action="delCliente" method="post" name="form_del_cliente" id="form_del_cliente" onsubmit="event.preventDefault(); delCliente();">'+
                                            '<h1>Eliminar Cliente</h1>'+
                                            '<h3>¿Está seguro que desea eliminar el siguiente cliente?</h3>'+
                                            '<h2 class="dniCliente">RUC: '+info.ruc+'</h2><br>'+
                                            '<h2 class="nameCliente">Razon Social: '+info.razon_social+'</h2><br>'+
                                            '<input type="hidden" name="del_cliente_id" id="del_cliente_id" value="'+info.idcliente+'"required>'+
                                            '<div class="alert alertDelProduct"></div>'+
                                            '<a href="#" class="btn-cancel" onclick="closemodal();"><i class="fas fa-ban"></i> Cancelar</a>'+
                                            '<button type="submit" class="btn-ok"><i class="far fa-trash-alt"></i> Eliminar</button>'+ 
    ...
    $(".del_user").click(function(e){
        ...
        $.ajax({
            ...
            if(response != 'error'){
                    var info = JSON.parse(response);
                    $('.bodyModal').html('<form action="delUser" method="post" name="form_del_usuario" id="form_del_usuario" onsubmit="event.preventDefault(); delUser();">'+
                                            '<h1>Eliminar Usuario</h1>'+
                                            '<h3>¿Está seguro que desea eliminar el siguiente Usuario?</h3>'+
                                            '<h2 class="nameUser">Nombre: '+info.nombre+'</h2><br>'+
                                            '<h2 class="userUser">Usuario: '+info.usuario+'</h2><br>'+
                                            '<h2 class="rolUser">Rol: '+info.rol+'</h2><br>'+
                                            '<input type="hidden" name="del_usuario_id" id="del_usuario_id" value="'+info.idusuario+'"required>'+
                                            '<div class="alert alertDelProduct"></div>'+
                                            '<a href="#" class="btn-cancel" onclick="closemodal();"><i class="fas fa-ban"></i> Cancelar</a>'+
                                            '<button type="submit" class="btn-ok"><i class="far fa-trash-alt"></i> Eliminar</button>'+ 
                                        '</form>');
    ...
    //Modal Form Delete Proveedor
    $(".del_proveedor").click(function(e){
        e.preventDefault();
        var prov = $(this).attr('proveedor');
        var action = 'infoProv';
        $.ajax({
            url: 'ajax.php',
            type: 'POST',
            async:true,
            data: {action:action,prov:prov},
            
            success: function(response){
            if(response != 'error'){
                    var info = JSON.parse(response);
                    $('.bodyModal').html('<form action="delProv" method="post" name="form_del_proveedor" id="form_del_proveedor" onsubmit="event.preventDefault(); delProv();">'+
                                            '<h1>Eliminar Usuario</h1>'+
                                            '<h3>¿Está seguro que desea eliminar el siguiente proveedor?</h3>'+
                                            '<h2 class="nameUser">Proveedor: '+info.proveedor+'</h2><br>'+
                                            '<h2 class="rolUser">Telefono: '+info.telefono+'</h2><br>'+
                                            '<input type="hidden" name="del_prov_id" id="del_prov_id" value="'+info.codproveedor+'"required>'+
                                            '<div class="alert alertDelProduct"></div>'+
                                            '<a href="#" class="btn-cancel" onclick="closemodal();"><i class="fas fa-ban"></i> Cancelar</a>'+
                                            '<button type="submit" class="btn-ok"><i class="far fa-trash-alt"></i> Eliminar</button>'+ 
                                        '</form>');
    ...
    //Activar campos para registro de cliente
    $('.btn_new_cliente').click(function(e){
        e.preventDefault();
        $('#nom_cliente').removeAttr('disabled');
        $('#ap_pat_cliente').removeAttr('disabled');
        $('#ap_mat_cliente').removeAttr('disabled');
        $('#raz_cliente').removeAttr('disabled');
        $('#dir_cliente').removeAttr('disabled');
        $('#div_registro_cliente').slideDown();
    });
    //Buscar cliente con API
    $(".btn_search_cliente").click(function(e){
        e.preventDefault();
        var action = "verDocumento";
        var dni = $(".dni_cliente").val();
        var tipo = $('.bol_fac').val();
        $.ajax({
            url: 'ajax.php',
            type: 'POST',
            async:true,
            data: {action:action,dni:dni,tipo:tipo},
            success: function(response){
                if(response !='error'){
                    var parser = JSON.parse(response);
                    var info = JSON.parse(parser);
                    $('#nom_cliente').val(info.result['nombres']);
                    $('#ap_pat_cliente').val(info.result['apellidoPaterno']);
                    $('#ap_mat_cliente').val(info.result['apellidoMaterno']);
                    $('#raz_cliente').val(info.result['razon_social']);
                    $('#dir_cliente').val(info.result['direccion']);
                }
            },
        ...
        $.ajax({
            ...
            success: function(response){
                if(response !='error'){
                    ...
                    $('#ap_pat_cliente').attr('disabled','disabled');
                    $('#ap_mat_cliente').attr('disabled','disabled');
                    $('#raz_cliente').attr('disabled','disabled');
    ...
    $('#btn_facturar_venta').click(function(e){
        ...
        if(rows > 0 ){
            ...
            $.ajax({
                ...
                type: "POST", 
                async:true, 
                data: {action:action,codcliente:codcliente,tipo:tipo,pago:pago,cuotas:cuotas,desc:desc},
    ...
    //Proformar Venta
    $('#btn_cotizar_venta').click(function(e){
        e.preventDefault();

        var rows = $('#detalle_venta tr').length;
        if(rows > 0 ){
            var action = 'procesarProforma';
            var codcliente = $('#idcliente').val();
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    if(response != 'error'){
                        var info = JSON.parse(response);
                        generarPDFProf(info.codcliente,info.noproforma);
                        location.reload();
    ...
    $(".anular-factura").click(function(e){
        e.preventDefault();
            ...
            success: function(response){
                if(response != 'error'){
                    ...
                    $('.bodyModal').html('<form action="" method="post" name="form_anular_factura" id="form_anular_factura" onsubmit="event.preventDefault(); anularFactura();">'+
                                            ...
                                            '<div class="alert alertAddProduct"></div>'+
                                            '<button type="submit" class="btn-new"><h3>Anular <i class="fa fa-trash"></i></h3></button>'+
                                           '<a href="#" class="btn-ok closemodal" onclick="closemodal();"><h3>Cerrar <i class="fa fa-ban"></i></h3></a>'+ 
                                        '</form>');
                }
            },
    ...
    //Ver Proforma
    $('.view-proforma').click(function(e){
        e.preventDefault();
        var codCliente = $(this).attr('cl');
        var noFactura= $(this).attr('f');
        generarPDFProf(codCliente,noFactura);
    });
    //Buscar Producto
    $('#txt_producto').keyup(function(e){
        e.preventDefault();
        var producto = $(this).val();
        var action = 'infoBuscar';
        $.ajax({
            url: 'ajax.php',
            type: "POST",
            async: true,
    ...
    //Activar Carrito
    $('.inp_cantidad').change(function(e){
        e.preventDefault();
        var cant =$(this).val();
        var id = $(this).attr('id');
        if(cant>0){
            $('.tab'+id+' .div-addcart').html('<button class="btn-view add_cart" onclick="add_cart('+id+');"type="button"><i class="fa fa-cart-shopping" style="font-size:18px;"></i></button>');
        }else{
            $('.tab'+id+' .div-addcart').html('<button class="btn-view add_cart inactivo" onclick="add_cart('+id+');"type="button" disabled><i class="fa fa-cart-shopping" style="font-size:18px;"></i></button>');
        }
    });
    $('.inp_cantidad').keyup(function(e){
        e.preventDefault();
        var cant =$(this).val();
        var id = $(this).attr('id');
        if(cant>0){
    ...
    //Eliminar Foto Especifica
    $('.del-Photo2').click(function(e){
        e.preventDefault();
        var idFoto = $('.inp-Photo2').val();
        var action = 'infoFoto';
        ...
        $.ajax({
            ...
            type: "POST",
            async: true,
            data: {action:action,idFoto:idFoto,idProducto:idProducto},
            success: function(response){
                if(response != 'error'){
                    var info = JSON.parse(response);
                    $('.modal').fadeIn(); 
                    $('.bodyModal').html('<form action="delPhoto" method="post" name="form_anular_factura" id="form_anular_factura" onsubmit="event.preventDefault(); eliminarFoto();">'+
                                            '<h1><i class="fas fa-cubes" style="font-size:45pt;"></i> <br>Eliminar foto</h1><br>'+
                                            '<p>¿Seguro que desea eliminar la foto?</p>'+
                                            '<p><strong>N°: '+info.num+'</strong></p>'+
                                            '<img src="img/uploads/'+info.foto+'" style="width: 200px; height: auto; margin:auto;"'+
                                            '<input type="hidden" name="action" value=eliminarFoto">'+
                                            '<input type="hidden" name="idFoto" id="idFoto" value="'+info.idfoto+'">'+
                                            '<div class="alert alertAddProduct"></div>'+
                                            '<button type="submit" class="btn-new"><h3>Eliminar Foto<i class="fa fa-trash"></i></h3></button>'+
                                           '<a href="#" class="btn-ok closemodal" onclick="closemodal();"><h3>Cerrar <i class="fa fa-ban"></i></h3></a>'+ 
                                        '</form>');
    ...
    $('.btn-chart').click(function(e){
        e.preventDefault();
        var btn = $(this).attr('cl');
        switch(btn){
            case 'btn-ingresos': $('.ganancias').attr('disabled',false); $('.ventas').attr('disabled',false); $('.top').attr('disabled',false); $(this).attr('disabled',true); break;
            case 'btn-ganancias': $('.ingresos').attr('disabled',false); $('.ventas').attr('disabled',false); $('.top').attr('disabled',false); $(this).attr('disabled',true); break;
            case 'btn-ventas': $('.ingresos').attr('disabled',false); $('.ganancias').attr('disabled',false); $('.top').attr('disabled',false); $(this).attr('disabled',true); break;
            case 'btn-top': $('.ingresos').attr('disabled',false); $('.ventas').attr('disabled',false); $('.ganancias').attr('disabled',false); $(this).attr('disabled',true); break;
        }
    });
    //Descontar Total
});
...
function add_cart(codproducto){
    var action = 'infoVenta';
    var producto = $('.tab'+codproducto+' .sku').html();
    var cantidad = $('.tab'+codproducto+' .cant').val();
    var exst = $('.tab'+codproducto+' .celExistencia').html();
    var precio = $('.tab'+codproducto+' .precio').val();
    ...
    var action = 'addProductoDetalle';
        $.ajax({
            ...
            type: "POST",
            async: true,
            data: {action:action,producto:producto,cantidad:cantidad,precio:precio,descuento:descuento},
            ...
            success: function (response){
                if(response != 'error'){
                    ...
                    $('.tab'+codproducto+' .cant').val(0);
                    $('.tab'+codproducto+' .div-addcart').html('<button class="btn-view add_cart inactivo" onclick="add_cart('+codproducto+');"type="button" disabled><i class="fa fa-cart-shopping" style="font-size:18px;"></i></button>');
                }
            },
            error: function (error){
            }
...
function actualizarDescuento(input){
    var descuento = $('#descuento').val();
    var montotal = parseFloat($('.montotal').attr('id'));
    montotal = (montotal-descuento).toFixed(2);
    $('.montotal').html(montotal);
   
}
function checkPago(checkbox){
    var action='actualizarTotal';
    if($('#pago_tarj').prop('checked')){
        var met='tarjeta';
        var tabla = document.getElementById("tbl_venta");
        var nprecio = 0;
        for(var i = 1; tabla.rows[i]; i++) {
        precio = parseFloat($('.inpPrecio').val());
        nprecio = (precio + precio*0.05).toFixed(2);
        tabla.rows[i].cells[2].innerHTML='<input class="inpnPrecio precio" type="number" value='+nprecio+'>';
        $('.div-cuotas').slideDown();
        }
    }else{
        var met='efectivo';
        var tabla = document.getElementById("tbl_venta");
        var nprecio = 0;
        for(var i = 1; tabla.rows[i]; i++) {
        precio = parseFloat($('.inpnPrecio').val());
        nprecio = (precio /1.05).toFixed(2);
        tabla.rows[i].cells[2].innerHTML='<input class="inpPrecio precio" type="number" value='+nprecio+'>';
        $('.div-cuotas').slideUp();
        }
    ...
    $.ajax({
        ...
        success:function(response){
            if(response =='error'){
                ...
                $('#form_anular_factura .btn-new').remove();
                $('#row_'+noFactura+' .div-factura').html('<button type="button" class="btn-anular inactivo"><i class="fas fa-ban"></i></button>');
...
//Cambio de Documento al Registrar Cliente
function cambioDocumento(){
    var id = $('.select-documento').val();
        if(id =='DNI'){ 
            var form = '<select class="select-documento" name="documento" id="documento" onchange="cambioDocumento()">'+
        ...
        if(id =='RUC'){
            ...
            '<input type="number" name="ruc" id="ruc" placeholder="10123456780">'+
            ...
            '<input type="text" name="direccion" id="direccion" placeholder="Av. Las Lomas 123">'+
            '<div class="alert"><?php echo isset($alert)  $alert :" "; ?></div>'+
            '<input class="btnRegister"type="submit" name="sumbit-client" value="Guardar Cliente">';
        }
            $('.form-new').html(form);
...
function bol_fac(){
  ...
  var form;
  if(tipo == 'Boleta'){
    form = '<input type="hidden" name="action" value="addCliente">'+
            '<input type="hidden" name="idcliente" id="idcliente" value="" required>'+  
            '<div class="wd30">'+
              '<label>DNI </label>'+
              '<input type="text" name="dni_cliente" id="dni_cliente" class="dni_cliente" onkeyup="searchCliente()">'+
            '</div>'+
            '<div class="wd30">'+
              '<label>Nombre </label>'+
    ...
    form =  '<input type="hidden" name="action" value="addCliente">'+
            ...
            '<div class="wd30">'+
            '<label>RUC </label>'+
            '<input type="text" name="ruc_cliente" id="ruc_cliente" class="dni_cliente" onkeyup="searchCliente()"> '+
            '</div>'+
            '<div class="wd30">'+
              '<label>Razon Social </label>'+
              '<input type="text" name="raz_cliente" id="raz_cliente" disabled required>'+
            '</div>'+
            '<div class="wd30">'+
  ...
  $('#form_new_cliente_venta').html(form);  
}
function searchCliente(){
    var cl = $('.dni_cliente').val();
    ...
    $.ajax({
        ...
        type: "POST",
        async: true,
        data: {action:action,cliente:cl,tipo:tipo},
        success: function(response){
            if(response == 0){
                $('#idcliente ').val('');
                $('#nom_cliente').val('');
                ...
                $('#ap_pat_cliente').val (data.apellido_paterno);
                $('#ap_mat_cliente').val (data.apellido_materno);
                $('#raz_cliente').val(data.razon_social);
...
function generarPDFProf(cliente,proforma){
    var ancho = 1000;
    var alto = 800;
    ...
    $url = 'factura/generaProforma.php?cl='+cliente+'&p='+proforma;
    window.open($url,"Proforma","left="+x+",top="+y+",height="+alto+"width="+ancho+",scrollbar=si,location=no,resizable=si,menubar=no");
...
function delProduct(){
    ...
    $.ajax({
            ...
            success: function(response){
                ...
                if(response == 'error'){
                    $('.alertDelProduct').html('<p style="color: red;">Error al eliminar el producto.</p>');
                }else{
                    $('.row'+pr).remove();
                    $('#form_del_product .btn-ok').remove();
                    $('#txtCantidad').val('');
...
//Eliminar Cliente
function delCliente(){
    var pr = $('#del_cliente_id').val();
    var action = 'delCliente';
    var del_cliente_id = $('#del_cliente_id').val();
    $('.alertDelProduct').html('');
    $.ajax({
            url: 'ajax.php',
            type: 'POST',
            async:true,
            data: {action:action,del_cliente_id:del_cliente_id},
            success: function(response){

                if(response == 'error'){
                    $('.alertDelProduct').html('<p style="color: red;">Error al eliminar el cliente.</p>');
                }else{
                    $('.row'+pr).remove();
                    $('#form_del_cliente .btn-ok').remove();
                    $('.alertDelProduct').html('<p>Cliente eliminado correctamente.</p>'); 
                }
            },
            error: function(error){
                console.log(error);
...
//Eliminar Usuario
function delUser(){
    var pr = $('#del_usuario_id').val();
    var action = 'delUser';
    var del_usuario_id = $('#del_usuario_id').val();
    $('.alertDelProduct').html('');
    $.ajax({
            url: 'ajax.php',
            type: 'POST',
            async:true,
            data: {action:action,del_usuario_id:del_usuario_id},
            success: function(response){
                console.log(response);
                if(response == 'error'){
                    $('.alertDelProduct').html('<p style="color: red;">Error al eliminar el usuario.</p>');
                }else{
                    $('.row'+pr).remove();
                    $('#form_del_usuario .btn-ok').remove();
                    $('.alertDelProduct').html('<p>Usuario eliminado correctamente.</p>'); 
                }
            },
            error: function(error){
                console.log(error);
...
//Eliminar Usuario
function delProv(){
    var pr = $('#del_prov_id').val();
    var action = 'delProv';
    var del_prov_id = $('#del_prov_id').val();
    $('.alertDelProduct').html('');
    $.ajax({
            url: 'ajax.php',
            type: 'POST',
            async:true,
            data: {action:action,del_prov_id:del_prov_id},
            success: function(response){
                console.log(response);
                if(response == 'error'){
                    $('.alertDelProduct').html('<p style="color: red;">Error al eliminar el proveedor.</p>');
                }else{
                    $('.row'+pr).remove();
                    $('#form_del_proveedor .btn-ok').remove();
                    $('.alertDelProduct').html('<p>Proveedor eliminado correctamente.</p>'); 
                }
            },
            error: function(error){
                console.log(error);
...
function eliminarFoto(){
    ...
    var action = 'eliminarFoto';

    $.ajax({
        url:'ajax.php',
        type:"POST",
        async:true,
        data:{action:action,idFoto:idFoto},

        success:function(response){
            if(response =='error'){
                $('.alertAddProduct').html('<p style = "color:red;">Error al eliminar la foto.</p>');
            }else{
                $('.alertAddProduct').html('<p>Foto eliminada.</p>');
            }
...
//Pasar Proforma a Venta
function proformaVenta(codcliente,noproforma){
    var action = 'seguirVenta';
    var codcliente = codcliente;
    var noproforma = noproforma;
    $.ajax({
        url: 'ajax.php',
        type: "POST",
        async: true,
        data: {action:action,codcliente:codcliente,noproforma:noproforma},
        success: function(response){
            if(response != 'error'){
                    var info = JSON.parse(response);
                    $('#detalle_venta').html(info.detalle);
                    ...
                    if(info.dni !=''){
                        ...
                        $('#ap_pat_cliente').val (info.apellido_paterno);
                        $('#ap_mat_cliente').val(info.apellido_materno);
                        $('.btn_new_cliente').slideUp();                  
                    }
                    if(info.ruc !=''){
                        form =  '<input type="hidden" name="action" value="addCliente">'+
                                ...
                                '<div class="wd50">'+
                                '<label>RUC </label>'+
                                '<input type="text" name="ruc_cliente" id="ruc_cliente" class="dni_cliente" value="'+info.ruc+'" onkeyup="searchCliente()"> '+
                                '</div>'+
                                '<div class="wd50">'+
                                  '<label>Razon Social </label>'+
                                  '<input type="text" name="raz_cliente" id="raz_cliente" value="'+info.razon_social+'"disabled required>'+
                                '</div>'+
                                '<div id="div_registro_cliente" class="wd50">'+
                                  '<br>'+
                                  '<button type="submit" class="btn-save"><i class="far fa-save"></i> Guardar</button>'+
...
//Ver Ubicacion  y Detalle
function viewUbicacion(codcliente,nofactura){
        var codCliente = codcliente;
        var noFactura= nofactura;
        var action = 'verDetalle';
        $.ajax({
            url: 'ajax.php',
            type: 'POST',
            async:true,
            data: {action:action,codCliente:codCliente,noFactura:noFactura},
            success: function(response){
                if(response != 'error'){
                    var info = JSON.parse(response);
                    var tabla = '';
                    console.log(info);
                    $('.bodyUbi').html('<table class="table-modal display">'+
                                                '<tr>'+
                                                    '<th>Sku</th>'+
                                                    '<th>Descripción</th>'+
                                                    '<th>Ubicacion</th>'+
...
//Activar Boton de Venta
function viewCantidad(){
    var cantidad = ('.inp_cantidad').val();
    if(cantidad=0){
